...
class DataPipeline:
    ...
    def save_data(self, df: pd.DataFrame, target_table_name: str) -> pd.DataFrame:
        ...
        sdf = sdf.withColumn(
            "update_timestamp_utc", sf.to_utc_timestamp(sf.current_timestamp(), "UTC")
        )
        sdf.write.mode("append").saveAsTable(f"{self._config.catalog_name}.{self._config.schema_name}.{target_table_name}")
